...
### 音效无法播放
*   确保浏览器允许自动播放音频
*   检查浏览器控制台是否有 Audio Context 相关错误
*   某些浏览器需要用户交互后才能播放音频，点击页面任意位置即可激活

### 在线对战先手问题
*   已修复：确保服务器正确发送 `currentTurn` 参数
*   如遇问题，检查 `server.js` 中的随机逻辑：`Math.random() < 0.5 ? 1 : 2`

### 技能能量扣除异常
*   已修复："飞沙走石" 技能能量扣除问题
*   如遇问题，检查 `script.js` 中的 `activateSkill` 函数
